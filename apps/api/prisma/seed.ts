...
async function main() {
  ...
  const potrero1 = await prisma.potrero.upsert({
    where: { id: 1 }, update: {},
    create: { nombre: 'Potrero Norte', area: 5.0, capacidad: 20, descripcion: 'Pasto kikuyo, zona alta' },
  })
  const potrero2 = await prisma.potrero.upsert({
    where: { id: 2 }, update: {},
    create: { nombre: 'Potrero Sur', area: 3.5, capacidad: 15, descripcion: 'Pasto brachiaria, zona baja' },
  })
  ...
  await prisma.vacunacion.createMany({
    data: [
      { animalId: lola.id, vacuna: 'Aftosa', dosis: '2ml', lote: 'L2024-01', fecha: new Date('2026-03-01'), proximaFecha: new Date('2026-09-01'), veterinario: 'Dr. Martínez', costo: 15000 },
      { animalId: lola.id, vacuna: 'Brucelosis', dosis: '1 dosis', fecha: new Date('2026-02-15'), veterinario: 'Dr. Martínez', costo: 20000 },
      { animalId: toro.id, vacuna: 'Aftosa', dosis: '2ml', lote: 'L2024-01', fecha: new Date('2026-03-01'), proximaFecha: new Date('2026-09-01'), veterinario: 'Dr. Martínez', costo: 15000 },
    ],
    skipDuplicates: true,
  })
  ...
  await prisma.eventoReproductivo.createMany({
    data: [
      { animalId: lola.id, tipo: TipoEvento.CELO, fecha: new Date('2026-04-05'), descripcion: 'Detección de celo por comportamiento' },
      { animalId: lola.id, tipo: TipoEvento.INSEMINACION, fecha: new Date('2026-04-06'), descripcion: 'Inseminación artificial', padreId: toro.id },
      { animalId: lola.id, tipo: TipoEvento.PRENEZ_CONFIRMADA, fecha: new Date('2026-05-15'), descripcion: 'Confirmado por palpación rectal', resultado: 'Positivo' },
    ],
    skipDuplicates: true,
  })
